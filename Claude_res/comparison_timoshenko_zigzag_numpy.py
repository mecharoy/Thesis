#Comparative analysis of Timoshenko vs Zigzag beam theory using NumPy/SciPy only
import numpy as np
...
def compute_zigzag_functions(z0, z1, z2, z3, Q55):
    """Compute zigzag function parameters according to the exact theory"""
    ...
    # Define reference layer (middle layer in this case)
    k0 = 1
    R2k0 = abs(R2[k0]) if abs(R2[k0]) > 1e-12 else 1.0
...
def compute_A_matrices_zigzag(Q11, Q55, z0, z1, z2, z3, R_k, dRz, b):
    ...
    def integrate_over_thickness(f, z_lower, z_upper, n_points=8):
...
def compute_A_matrices_timoshenko(Q11, Q55, z0, z1, z2, z3, b):
    ...
    # Where phi is the rotation of cross-section
    h = z3 - z0

    # Direct analytical integration for uniform beam
    A11 = Q11 * h * b          # ∫Q11*dA
    A12 = 0.0                    # ∫Q11*z*dA = 0 for symmetric cross-section
    A13 = Q55 * h**3 * b / 12.0  # ∫Q55*z*dA for Timoshenko
    A22 = Q11 * h**3 * b / 12.0  # ∫Q11*z^2*dA
    A23 = 0.0                    # ∫Q11*z*R_k(z)*dA = 0 for Timoshenko
    A33 = Q11 * h**3 * b / 12.0  # ∫Q11*R_k(z)^2*dA where R_k(z) = z for Timoshenko
    A33_bar = Q55 * h * b      # ∫Q55*(dR/dz)^2*dA where dR/dz = 1 for Timoshenko

    return A11, A12, A13, A22, A23, A33, A33_bar

def compute_element_stiffness_matrix(A11, A12, A13, A22, A23, A33, A33_bar, C1, C5, C6, C7):
...
def assemble_global_stiffness_matrix(x_coords, element_lengths, params, theory_type='zigzag'):
    ...
    for e in range(num_elements):
        # Use specific length and midpoint for this element
        ...
        K_e = compute_element_stiffness_matrix(A11, A12, A13, A22, A23, A33, A33_bar, C1, C5, C6, C7)
...
def compute_inertia_integrals_timoshenko(rho, b, z0, z1, z2, z3):
    ...
    I00 = rho * h * b
    I01 = 0.0  # Symmetric cross-section
    I02 = rho * h**3 * b / 12.0  # ∫rho*z*dA
    I11 = rho * h**3 * b / 12.0  # ∫rho*z^2*dA
    I12 = 0.0  # Symmetric cross-section
    I22 = rho * h**3 * b / 12.0  # ∫rho*z^2*dA where z = R_k(z) for Timoshenko
    Ihat = I00
...
def get_response_at_specific_point(x_coords, u_full, target_x, target_z, params, theory_type='zigzag'):
    ...
    if hasattr(x_coords, 'toarray'):
        x_coords_np = x_coords.toarray().flatten()
    else:
    ...
    if len(u_full.shape) > 1:
        ...
        u0 = np.array([u_full[node_idx*4]])   # Axial displacement at mid-plane
        w0_x = np.array([u_full[node_idx*4+2]]) # Rotation (w0,x)
        psi0 = np.array([u_full[node_idx*4+3]])   # Shear rotation
...
def create_uniform_mesh(L, num_elements=2000):  # Reduced for faster computation
...
def generate_excitation_signal(time_array, active_duration, frequency):
    """Generate excitation signal with Hanning window"""
    active_mask = time_array <= active_duration
    signal = np.zeros_like(time_array, dtype=dtype)

    if np.sum(active_mask) > 1:
        active_time = time_array[active_mask]
        n = np.arange(np.sum(active_mask), dtype=dtype)
        hanning = 0.5 * (1 - np.cos(2 * np.pi * n / (np.sum(active_mask) - 1)))
        signal[active_mask] = hanning * np.sin(2 * np.pi * frequency * active_time)

    return signal

def run_time_integration(K, M, dt, num_steps, excitation_func, d1_node, d2_node, h, save_interval=50):
    """Run time integration using Newmark-beta method"""
    n_dofs = M.shape[0]
    ...
    # Factorize the effective stiffness matrix
    try:
        K_lu = lu_factor(K_eff)
        use_lu = True
    except:
        print("Warning: LU factorization failed, using direct solve")
        use_lu = False
    ...
    # Storage for results
    n_save = num_steps // save_interval + 1
    u_saved = np.zeros((n_dofs, n_save), dtype=dtype)
    save_times = []

    print(f"Starting time integration with {n_dofs} DOFs...")

    for step in range(num_steps):
        # External force vector
        F = np.zeros(n_dofs, dtype=dtype)
        F_ext = excitation_func(step)

        if d1_node < n_dofs and d1_node >= 0:
            F[d1_node] = F_ext
            if d1_node + 2 < n_dofs: F[d1_node + 2] = -F_ext * (h/2)

        if d2_node < n_dofs and d2_node >= 0:
            F[d2_node] = -F_ext
            if d2_node + 2 < n_dofs: F[d2_node + 2] = F_ext * (h/2)

        # Effective force
        F_eff = F + M @ (a0*u + a2*v + a3*a)

        # Solve for displacement
        if use_lu:
            u_new = lu_solve(K_lu, F_eff)
        else:
        ...
        if step % save_interval == 0:
            u_saved[:, step // save_interval] = u
            save_times.append(step * dt)
    ...
    return u_saved, np.array(save_times)
...
def run_comparative_analysis(L=2.0, E=70e9, rho=2700):
    ...
    # Create uniform mesh (no notch) - reduced for faster computation
    x_coords, element_lengths, min_dx = create_uniform_mesh(L, num_elements=1000)
    num_elements = len(element_lengths)
    ...
    total_duration = 200e-6  # Reduced for faster computation
    active_duration = 30e-6   # Reduced for faster computation
    ...
    # Response points
    response_points = [0.5, 1.0, 1.5, 2.0]  # Multiple points along beam
    ...
    for theory_type in ['zigzag', 'timoshenko']:
        ...
        K = assemble_global_stiffness_matrix(x_coords, element_lengths, params, theory_type)
        M = assemble_global_mass_matrix(x_coords, element_lengths, params, theory_type)
        ...
        d1_node = np.argmin(np.abs(x_coords - 0.6535))
        d2_node = np.argmin(np.abs(x_coords - 0.6465))
        d1_node_adj = d1_node * 4
        d2_node_adj = d2_node * 4

        # Time array for excitation
        time_array = np.arange(num_steps) * dt

        def excitation_func(step):
            if step < int(num_steps * active_duration / total_duration):
                t = step * dt
                n = int(num_steps * active_duration / total_duration)
                if n > 1:
                    n_local = step
                    hanning = 0.5 * (1 - np.cos(2 * np.pi * n_local / (n - 1)))
                    return hanning * np.sin(2 * np.pi * f * t)
            return 0.0

        # Run time integration
        start_time = python_time.time()
        u_saved, save_times = run_time_integration(
            K_dense, M_dense, dt, num_steps, excitation_func, d1_node_adj, d2_node_adj, h
        )
        end_time = python_time.time()

        print(f"{theory_type.capitalize()} analysis completed in {end_time - start_time:.2f} seconds")

        # Extract responses at specified points
        responses = {x_point: [] for x_point in response_points}
        for x_point in response_points:
            for i, t_val in enumerate(save_times):
                u_response, _ = get_response_at_specific_point(
                    x_coords, u_saved[:, i], x_point, target_z, params, theory_type
                )
                responses[x_point].append(u_response)

        results[theory_type] = {
            'responses': responses,
            'time_array': save_times
        }
